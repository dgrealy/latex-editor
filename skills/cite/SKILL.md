...
3. Key it `<firstauthorlastname><year><disambiguator>`, lowercase, e.g.
   `smith2019`, `smith2019a`.
...
5. Record provenance in `.latex-editor/bib-provenance.json`:
   `{"smith2019": {"source": "crossref", "url": "...", "fetched": "<date>", "verified": true}}`
...
turning "as Smith showed" into "as shown in \cite{smith2019}" deletes their
...
```
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/claude_commit.py "cite: add smith2019, fill 2 citations in methods" <bib> <tex files>
```
